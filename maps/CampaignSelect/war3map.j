...
function CreateUnitsForPlayer0 takes nothing returns nothing
    ...
    set gg_unit_e000_0000=CreateUnit(p, 'e000', 1536.0, 768.0, 20.633)
endfunction
...
function release_switch takes nothing returns nothing
    ...
    if GetPlayerTechMaxAllowed(Player(0), 200 + region_index) == AVAIL_BEATEN then
        call SetDestructableAnimation(s__switches[region_index], "Stand alternate")
    else
    ...
    loop
        ...
        loop
            ...
            if mission_availability >= AVAIL_AVAILABLE then
                if s__mission_select_regions[index] == null then
                    ...
                    set s__mission_select_regions[index]= new_region
                    // trigger
                    call TriggerRegisterEnterRegion(press_switch_t, new_region, Condition(function filter_unit_belongs_to_player))
                    call TriggerRegisterLeaveRegion(release_switch_t, new_region, Condition(function filter_unit_belongs_to_player))
                endif
            endif
            if mission_availability >= AVAIL_LOCKED then
                // destructable
                if s__switches[index] == null then
                    set new_switch=CreateDestructable('DTfx', GetRectCenterX(first_region) + offsetx, GetRectCenterY(first_region) + offsety, 0.0, 1.0, 0)
                    set s__switches[index]= new_switch
                else
                    set new_switch=s__switches[index]
                endif
                // animation
                if mission_availability == AVAIL_LOCKED then
...
function Trig_Aesthetics_Actions takes nothing returns nothing
    ...
    call CreateTextTagLocBJ("TRIGSTR_025", GetRectCenter(gg_rct_under_construction), 12.00, 10, 100, 100, 0.00, 100.00)
    call SetPlayerName(Player(1), "TRIGSTR_026")
    call SetPlayerName(Player(1), "TRIGSTR_027")
    set udg_adventurer=gg_unit_e000_0000
endfunction

//===========================================================================
function InitTrig_Aesthetics takes nothing returns nothing
    set gg_trg_Aesthetics=CreateTrigger()
    call TriggerRegisterTimerEventSingle(gg_trg_Aesthetics, 0.00)
...
function config takes nothing returns nothing
    ...
    call DefineStartLocation(2, 1344.0, 832.0)

    // Player setup
    call InitCustomPlayerSlots()
    call SetPlayerSlotAvailable(Player(0), MAP_CONTROL_USER)
    call SetPlayerSlotAvailable(Player(1), MAP_CONTROL_NEUTRAL)
    call SetPlayerSlotAvailable(Player(2), MAP_CONTROL_NEUTRAL)
